var GLOBAL = this, JSLoader = function(config) {
	config = config || {};
	var defaultLoadNames = config.defaultLoadNames || [];
	var _serviceVersion = null;
	if (!config.lang) {
	...
	try {
		if (_localStorage) {
			...
			if (versionTxt === null) {
				...
				_localStorage.clear();
				_serviceVersion = new Date().getTime();
				...
				if (versionTxt || winxhr.status == 200) {
					...
					_serviceVersion = encodeURIComponent(serviceVersion);
		...
		return {
			...
			resolve : function(name, useGlobal) {
				...
				if (useGbk) {
					...
					if (!window.bof_offline_xhr) { //don't set server version in offline mode for the '&' sign will affect path resolving
						url += "&t=" + _serviceVersion;
					}
				}
	...
	var jsloader = function() {
		...
		loaderChain[1].resolveMany([ 'freequery.lang.lang', 'freequery.lang.domutils',
				...
				'thirdparty.jquery.jquery', 'thirdparty.layer.layer', 'freequery.common.DealBackspaceKey' ].concat(defaultLoadNames));
		// --StaticLoaderEnd-- //
	...
	var DealBackspaceKey = jsloader.resolve("freequery.common.DealBackspaceKey");
	//执行构造方法 阻止backspace按键后退事件
	new DealBackspaceKey();
	// --DefaultImportsEnd--//
